...
from scripts.infere_zeroshot import render_interpolate_camera
...
class RenderDatasets():
    ...
    def main(self):
        ...
        pipeline.model.field.use_average_appearance_embedding = False

        if pipeline.datamanager.dataparser.config.drop50:
            test_orderInTrainlist = [2, 4, 8, 10, 12, 14,18, 20, 22, 24, 28, 30]
        elif pipeline.datamanager.dataparser.config.drop80:
            pipeline.model.field.binocular = True
            test_orderInTrainlist = [2, 2, 4, 4, 6, 6, 8, 8, 10, 10, 12, 12]
        elif pipeline.datamanager.dataparser.config.drop90:
            pipeline.model.field.binocular = True
            test_orderInTrainlist = [2, 2, 2, 2, 4, 4, 4, 4, 6, 6, 6, 6]
            # coeff = [0.9,0.7,0.3,0.1,0.9,0.7,0.3,0.1,0.9,0.7,0.3,0.1]
            # pipeline.model.field.coeff = coeff
            # test_orderInTrainlist = [0,0,2,2,2,2,4,4,4,4,6,6]
        ...
        if self.task == 'trainset':
            ...
            if self.is_leaderboard:
                ...
                pipeline.model.field.use_average_appearance_embedding = False
                Test_orderInTrainlist = test_orderInTrainlist 
                # Test_orderInTrainlist = [2+2*i for i in range(15)] 

                # Test_orderInTrainlist = [10,11]  ## waymo
