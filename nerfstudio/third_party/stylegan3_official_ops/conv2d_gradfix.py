...
def conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1,impl=None):
    if _should_use_custom_op(input):
        return _conv2d_gradfix(transpose=False, weight_shape=weight.shape, stride=stride, padding=padding, output_padding=0, dilation=dilation, groups=groups).apply(input, weight, bias)
    return torch.nn.functional.conv2d(input=input, weight=weight, bias=bias, stride=stride, padding=padding, dilation=dilation, groups=groups)

def conv_transpose2d(input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1,impl=None):
    if _should_use_custom_op(input):
...
def _conv2d_gradfix(transpose, weight_shape, stride, padding, output_padding, dilation, groups):
    ...
    class Conv2d(torch.autograd.Function):
        @staticmethod
        def forward(ctx, input, weight, bias, impl=None):
    ...
    class Conv2dGradWeight(torch.autograd.Function):
        @staticmethod
        def forward(ctx, grad_output, input, weight, impl=None):
            ctx.save_for_backward(
