...
class Pipeline(nn.Module):
    ...
    def get_eval_zeroshot(self, step: int):
        """This function gets your evaluation loss dict. It needs to get the data
        from the DataManager and feed it to the model's forward function
...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        self.datamanager: VanillaDataManager = config.datamanager.setup(
            device=device, test_mode=test_mode, world_size=world_size, local_rank=local_rank,config_path=config_path
        )
        ...
        self._model = config.model.setup(
            ...
            intrinsics = self.datamanager.train_dataset.cameras.get_normal_intrinsics_matrices(),
            num_train_data=len(self.datamanager.train_dataset),
            metadata=self.datamanager.train_dataset.metadata,
            world_size=world_size,
            local_rank=local_rank,
            volume_dict = self.datamanager.train_dataset._dataparser_outputs.volume_dict,
            config_path = config_path,
            lidar_data = self.datamanager.train_dataset._dataparser_outputs.lidar_data,
            device = device,
        )
    ...
    def get_train_loss_dict(self, step: int):
        ...
        model_outputs = self._model(ray_bundle,batch.get('mask',None))
        metrics_dict = self.model.get_metrics_dict(model_outputs, batch)
        ...
        loss_dict = self.model.get_loss_dict(model_outputs, batch, metrics_dict,step,ray_bundle)
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        """
        self.eval()
        image_idx, camera_ray_bundle, batch,scene_id= self.datamanager.next_eval_image(step)
        camera_ray_bundle.scene_id = torch.tensor([[scene_id]]).long()  
    ...
    def get_eval_zeroshot(self, step: int):
        ...
        """
        self.eval()
        image_idx, camera_ray_bundle, batch,scene_id= self.datamanager.next_fixed_eval_image(step=step,scene_id=0)
        camera_ray_bundle.scene_id = torch.tensor([[scene_id]]).long()  
        outputs = self.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle)
        # outputs = self.model.get_outputs_for_fixed_ray_bundle(camera_ray_bundle)
        return outputs, batch['image']
