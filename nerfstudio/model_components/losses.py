...
def Hierarchy_distortion_loss(weights, ray_samples):
    """From mipnerf360"""
...
def urban_radiance_field_depth_loss(
    ...
    line_of_sight_loss_near_mask = torch.logical_and(
     ...
     ## analog to Gaussion Normal
    # pdf = torch.exp(target_distribution.log_prob(steps - termination_depth))
    # interval = steps[...,1:,0] - steps[...,:-1,0]
    # padding = torch.full((interval.shape[0], 1), 1e-4).to(interval)
    # interval = torch.cat([padding,interval],dim=1).unsqueeze(dim=-1)
    # line_of_sight_weight_gt = pdf * interval
    # line_of_sight_loss_near = (weights - line_of_sight_weight_gt) ** 2
    # line_of_sight_loss_near = (line_of_sight_loss_near_mask * line_of_sight_loss_near).sum(-2)

    ## No strict to Gaussion Distribution
    line_of_sight_weight = torch.sum(line_of_sight_loss_near_mask * weights,dim=1)
    line_of_sight_loss_near = torch.ones_like(line_of_sight_weight) - line_of_sight_weight
    line_of_sight_loss_near = line_of_sight_loss_near**2
...
def fg_bg_entroyloss(mask_pred_cr):
    eps = 1e-5
