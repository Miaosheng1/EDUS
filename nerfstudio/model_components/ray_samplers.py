...
class SpacedSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        bins = torch.linspace(0.0, 1.0, num_samples + 1)[None, ...].to(ray_bundle.fars.device)  # [1, num_samples+1]
...
class HierarchyImportanceSampler(Sampler):
    """NeuS sampler that uses a density value to generate importance samples """
    ...
    def generate_ray_samples(
        self,
        ray_bundle: Optional[RayBundle] = None,
        density_fn: Optional[Callable[[RaySamples], torch.Tensor]] = None,
        ray_samples: Optional[RaySamples] = None,
        **kwargs,
    ) -> Union[Tuple[RaySamples, torch.Tensor], RaySamples]:
        ...
        ## 3dcnn 
        feat_volume = kwargs.get("feat_volume",None)

        if ray_samples is None:
            ray_samples = self.initial_sampler(ray_bundle, num_samples=self.num_samples)
            uniform_sample = ray_samples
        assert ray_samples is not None
        ...
        while total_iters < self.num_upsample_steps:
            with torch.no_grad():
                new_density, _ = density_fn(new_samples,scene_id=scene_id,feats_volume=feat_volume)

            # merge density predictions
            if sorted_index is not None:
                ## concat density
                density_merge = torch.cat([density.squeeze(-1),new_density.squeeze(-1)],dim=-1)
                density = torch.gather(density_merge, 1, sorted_index).unsqueeze(-1)
            else:
                density = new_density
            ...
            weights = torch.cat((weights, torch.zeros_like(weights[:, :1])), dim=1)
            annealed_weights = torch.pow(weights, self._anneal)
            # annealed_weights = torch.pow(weights, 1)
            new_samples = self.pdf_sampler(
                ray_bundle,
                ray_samples,
                annealed_weights,
        ...
        return ray_samples,uniform_sample
