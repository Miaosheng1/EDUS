...
class RayBundle(TensorDataclass):
    ...
    source_images = None
    source_depth = None
    scene_id: Optional[TensorType[..., 1]]  = None
    ...
    def set_source_image(self,source_images,source_poses,source_depth):
    ...
    def Concat_RayBundle(self,RayBundle_A, RayBundle_B) -> "RayBundle":
        ...
        raybundle = RayBundle(
            origins= torch.concat([RayBundle_A.origins,RayBundle_B.origins],dim=0),
            directions= torch.concat([RayBundle_A.directions,RayBundle_B.directions],dim=0),
            pixel_area=None,
            camera_indices=torch.concat([RayBundle_A.camera_indices,RayBundle_B.camera_indices],dim=0),
            directions_norm= torch.concat([RayBundle_A.directions_norm,RayBundle_B.directions_norm],dim=0),
            times=None,
            nears= torch.concat([RayBundle_A.nears,RayBundle_B.nears],dim=0),
            fars= torch.concat([RayBundle_A.fars,RayBundle_B.fars],dim=0),
        )

        return raybundle
    
    ## 将Mask 为True 的Raybundle 替换掉
    def Replace_Mask_Raybundle(self,RayBundle_A,Mask)-> "RayBundle":
        valid_index = torch.nonzero(Mask).long().squeeze()
        self.fars[valid_index] = RayBundle_A.fars
        return self
