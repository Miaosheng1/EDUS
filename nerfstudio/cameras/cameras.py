...
class Cameras(TensorDataclass):
    ...
    def get_image_coords(
        self, pixel_offset: float = 0.5, index: Optional[Tuple] = None, downscale:float = 1,
        ...
        if index is None:
            ...
            image_coords = torch.meshgrid(torch.arange(0,image_height,downscale), torch.arange(0,image_width,downscale), indexing="ij")
    ...
    def generate_rays(  # pylint: disable=too-many-statements
        ...
        if coords is None:
            ...
            coords: torch.Tensor = cameras.get_image_coords(index=tuple(index),downscale= downscale)  # (h, w, 2)
    ...
    def get_intrinsics_matrices(self) -> TensorType["num_cameras":..., 3, 3]:
        """Returns the 3×3 intrinsic matrices for each camera.
    ...
    def get_normal_intrinsics_matrices(self) -> TensorType["num_cameras":..., 4, 4]:
        """Returns the 4×4 intrinsic matrices for each camera.
