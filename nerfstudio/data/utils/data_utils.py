...
def get_image_mask_tensor_from_path(filepath: Path, scale_factor: float = 1.0) -> torch.Tensor:
    ...
    if pil_mask.shape[-1] == 3:
        pil_mask = pil_mask[...,0]
    mask_tensor = torch.from_numpy(pil_mask).unsqueeze(-1)
    return mask_tensor

def get_image_semantics_tensor_from_path(filepath: Path, scale_factor: float = 1.0) -> torch.Tensor:
    """
    Utility function to read a mask image from the given path and return a boolean tensor
    """
    ...
    if scale_factor != 1.0:
        ...
        newsize = (int(width * scale_factor), int(height * scale_factor))
        pil_mask = pil_mask.resize(newsize, resample=Image.NEAREST)
    semantic_tensor = torch.from_numpy(np.array(pil_mask)).unsqueeze(-1)
